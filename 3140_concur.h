...
{			 //PROCESS STATE BEGIN
	unsigned int sp; // stack pointer related to related process
	lock_t *owner; // set the lock that this process is waiting on
}; 			 //PROCESS STATE END

// create l_lock
struct lock_impl
{
	process_t *owned; // set the process that owns the lock
};
/*------------------------------------------------------------------------
